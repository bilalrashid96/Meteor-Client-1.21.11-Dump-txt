...
public class FakePlayerCommand extends Command {
   ...
   public void build(LiteralArgumentBuilder<class_2172> builder) {
      builder.then(((LiteralArgumentBuilder)literal("add").executes((context) -> {
         ...
         FakePlayerManager.add(fakePlayer.name.get(), (float)(Integer)fakePlayer.health.get(), (Boolean)fakePlayer.copyInv.get());
         return 1;
      ...
      builder.then(literal("remove").then(argument("fp", FakePlayerArgumentType.create()).executes((context) -> {
         ...
         if (fp != null && FakePlayerManager.contains(fp)) {
            ...
            this.info("Removed Fake Player %s.".formatted(fp.method_5477().getString()), new Object[0]);
            return 1;
         } else {
      ...
      builder.then(literal("list").executes((context) -> {
         ...
         FakePlayerManager.forEach((fp) -> ChatUtils.info("(highlight)%s".formatted(fp.method_5477().getString())));
         return 1;
      }));
   }
}
